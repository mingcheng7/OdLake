...
function[zz,Az,Vz,tt,Qst,Kzt,Tzt,rhozt,DOzt,PO4zt,NO3zt,NH3zt,CyBzt,APSBzt,H2Szt,SO4zt,DFezt,PFezt,DMnzt,PMnzt,PARzt,Wt]...
    = odlake_v0_1_SR_exp(Datetime,Initfile,Initsheet,Inputfile,Inputsheet,Parafile,Parasheet,r_Qsw,varargin)

% Inputs (to solvemodel function) 
% Datetime: Model run date [year, month, day]
% + input filenames and sheetnames 
% r_Qsw   : A ratio number to control the value of surface radiation 
...
disp(['Running OdLake on ' datestr(datenum(Datetime))  ' ...']);  % Display model running date 
...
n=2500; % setting the number of iterated days 
...
if (nargin>8) %if optional command line parameter input is used 
 ...
 [tt,In_Z,In_A,In_T,In_DO,In_PAR,In_PO4,In_NO3,In_NH3,In_CyB,In_APSB,In_H2S,In_SO4,In_DFe,In_Fe3,In_MFe,In_DMn,In_PMn,...
...
% Read input data
  [tt,In_Z,In_A,In_T,In_DO,In_PAR,In_PO4,In_NO3,In_NH3,In_CyB,In_APSB,In_H2S,In_SO4,In_DFe,In_Fe3,In_MFe,In_DMn,In_PMn,...
    In_DFe_sed,In_DMn_sed,In_NO3_sed,In_SO4_sed,In_H2S_sed,In_PO4_sed,Wt,Inflw,Phys_par,Phys_par_range,Phys_par_names,Bio_par,Bio_par_range,Bio_par_names] ...
    = modelinputs(Datetime,Initfile,Initsheet,Inputfile,Inputsheet,Parafile,Parasheet,dt,n);
end

%load albedot1 table, in order to save execution time
...
Kz_exp=0.2; % This is the testing value, defult is 0.43 

% ice & snow parameter values
Tf=0;               % water freezing point temperature (deg C)
ksw=1e-3;           % Porewater_water mass transfer coefficient (m d-1)
...
NO3z = interp1(In_Z,In_NO3,zz+dz/2);             % Initial dissolved NO3 distribution (mol m-3) 
...
NO3_sed = interp1(In_Z,In_NO3_sed,zz+dz/2);      % Initial NO3 distribution in porewater (mol m-3) 
...
for i=1:length(tt)
    ...
    Qsw = Qsw*r_Qsw; % A specific surface radiation used in sensitivity tests 
    ...
    rho=polyval(dtf,max(0,Tz(:)))+min(Tz(:),0); % Water density (kg m-3) 
    ...
    % Inflw(:,5) Inflow dissolved NO3  concentration (mg m-3) 
    ...
    if (river_inflow_switch==1)
        ...
        Iflw_NO3 = Inflw(5);  % inflow NO3 concentration
        ...
        if (Iflw>0)
            if (isnan(Iflw_T))
                ...
                rho=polyval(dtf,max(0,Tz(:)))+min(Tz(:),0);
                rho_Iflw=polyval(dtf,max(0,Iflw_T))+min(Iflw_T,0);
            ...
            NO3z = IOflow(dz,zz,Vz,NO3z,lvlD,Iflw,Iflw_NO3);  % Dissolved NO3
    ...
    while (WmixIndicator==1)
        ...
        if (isempty(idx)==0); % The water layers are not fully mixed 
            ...
            if (KP_ratio>=1)
                ...
                rho = polyval(dtf,max(Tz(:),0)) + min(0,Tz(:));
                TKE = TKE - POE; 
    ...
    rho = polyval(dtf,max(0,Tz(:))) + min(0,Tz(:));
...
% advection-diffusion equation (tridiag_HAD_v11) by fully implicit hybrid exponential numerical scheme, 
...
% code from MyLake v.1.2 (Saloranta and Anderson, 2007)
